...
/**
 * Plus Mention (+) Autocomplete (v1)
 * 
 * Plus mention (+) is a prompt-native invocation operator for intelligence.
 * Currently resolves: Private vault items (files, folders, bio)
 * Future: Users, products, places, actions
 * 
 * The + symbol tells the system: "Bind this token to context, memory, and action"
 * 
 * At mention (@) = social mention system for people
 * Plus mention (+) = prompt-native invocation for intelligence
 */
...
interface UsePlusMentionAutocompleteReturn {
  isOpen: boolean;
  query: string;
  items: VaultItem[];
...
export function usePlusMentionAutocomplete(
  textareaRef: RefObject<HTMLTextAreaElement | null>,
  onInsertToken: (token: string, item: VaultItem) => void
): UsePlusMentionAutocompleteReturn {
